...
class Pulse():
    def __init__(self, gpioOn=None, gpioOff=None, usDelay=None, flags=None):
        self._gpioOn=gpioOn
        self._gpioOff=gpioOff
        self._usDelay=usDelay
        self._flags=flags

class Pigpio():
    """Driver for Wave Pulse on a GPIO Pin"""
    def __init__(self, gpiopin=None, output_type=1):
    ...
    def gpioWaveAddGeneric(self, pulse_count=pulse_count, pulses=pulses):
        for pulse in pulses:
    ...
    def gpioWaveCreate(self):
        wave_id = len(self._waves)
        self._waves.append(self._pulses)
        self._pulses = []
    ...
    def gpioWaveTxSend(self, wave_id=None, wave_mode=0):
        # send waves which contains Pulses
        wave = self._waves.pop(wave_id)
        self._busy = True

        return len(self._waves)-1

    def gpioWaveTxBusy(self):
        #check Wave sending (through boolean ?)
        return self._busy

    def gpioWaveDelete(self, wave_id):
        self._waves.pop(wave_id)
        return len(self._waves)-1
...
class IrSender():
    def __init__(self, gpio_pin, protocol, protocol_config, log_level = LogLevel.Minimal):
        ...
        self.pigpio = Pigpio(self.gpio_pin, PI_OUTPUT)
        ...
        if protocol == "NEC":
            ...
            raise NotImplementedError("Protocol not specified! Exiting...")
